...
import { ApiDefinition } from 'bravado-core';
...
const buildApis = async (apis) => {
  ...
  await forEachSerialP(async (item) => {
    ...
    if ('string' === typeof item) {
      builtApi = await ApiDefinition.build(item);
    } else {
      builtApi = await ApiDefinition.build(item.root, {
...
export const start = async function(apis, options = {}) {
  ...
  server.bravadoApiDefinition = defs.map((def) => def[1]);
  return server;
};
