...
module.exports = {
  start: async function(apis, options) {
    ...
    const corsMap = {};
    keys(routes).sort().reverse().forEach((routePath) => {
      corsMap[routePath] = { shouldSet: true, expectedHeaders: new Set(), allowedMethods: new Set(['OPTIONS']) };
      routes[routePath].forEach((action) => {
        if (action.method === 'opts') { corsMap[routePath].shouldSet = false; }
        corsMap[routePath].allowedMethods.add(action.method.toUpperCase());
        (action.expectedHeaders || []).forEach((header) => {
          corsMap[routePath].expectedHeaders.add(header);
        });
    ...
    keys(corsMap).forEach((route) => {
      const opts = cors.preflight(corsMap[route]);
      if (opts) { server.opts(route, opts); }
    });
