...
module.exports = {
  start: async function(apis, options) {
    ...
    keys(routes).sort().reverse().forEach((routePath) => {
      corsMap[routePath] = { shouldSet: true, expectedHeaders: [], allowedMethods: ['OPTIONS'] };
      routes[routePath].forEach((action) => {
        if (action.method === 'opts') { corsMap[routePath].shouldSet = false; }
        corsMap[routePath].allowedMethods.push(action.method.toUpperCase());
        corsMap[routePath].expectedHeaders.concat(action.expectedHeaders);
    ...
    keys(corsMap).forEach((route) => {
      cors.preflight({ path: route, ...corsMap[route] });
    });
