...
var schemasRouteHandler = require('./schemas-route');
    ...
    return loadPromise
      .then(function (defs) {
        ...
        defs.forEach(function (def) {
          ...
          for (var r in def.resources) {
            ...
            for (var a in resource.actions) {
              ...
              routes[routePath].push({
                ...
                handler: routeHandler(def, r, a, require(controllerPath))
              });
            }
          }
          routes[def.basePath] = [{
          ...
          routes[def.basePath + '_schemas'] = [{
            name: 'schemas',
            method: 'get',
            version: def.info.version,
            handler: schemasRouteHandler(def)
          }];
        });
