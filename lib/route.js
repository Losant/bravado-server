import buildBravadoHandler from 'bravado-core';
import { EventEmitter } from 'events';
import _debug from 'debug';
const debug = _debug('bravado:server:route');
import omnibelt from 'omnibelt';
const { omit, clone } = omnibelt;

export default function(api, resourceName, actionName, controller, baseContext, errorTransform) {
  const resource = api.resources[resourceName];
  const action = resource.actions[actionName];
  const handler = buildBravadoHandler(api, resourceName, actionName, controller);
  return function(req, res, next) {
    ...
    const _done = function(err, response) {
      ...
      if (err) {
        ...
        const statusCode = context.statusCode >= 300 && context.statusCode < 400 ? context.statusCode : 302;
        res.redirect(statusCode, response.toString(), next);
      } else {
